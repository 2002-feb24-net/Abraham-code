...
namespace Demo
{
    class Program
    {
        static void Main(string[] args)
        {
             ...
             * or maybe, to share the same data source across multiple copies of the running
             * program.
             * 
             * One way we can achieve that is with serialization to files like we coded last
             * week, with XML, JSON, etc.
             * 
             * Way 1 to store data: 
             *  C# objects (reference types, value types, fields/properties have object-to-object
             *  references.. collections (convenient, works well with code, but not persistent)
             * Way 2 to store data:
             *  Serialized as XML, JSON, or something similar (persistent, but awkward)
             * Way 3 to store data:
             *  Relational Database (SQL database). It's persistent, but it's very well optimized 
             *  for querying data without having to load anything beyond exactly the data you want.
             *  Optiized for making changes quickly
             *  
             * How relational databases treat data, what are basic building blocks tables, rows, and 
             * columns. One table for each kind of data. A table has one colume for each property that 
             * kind of data can have. Each row has some value for each column, because it represents one
             * instance of that kind of data
             * 
             * Because tables, rows, and columns are quite different to how C# represents objects, just
             * like how we used a serializer library (System.Text.Json) to handle the translation details,
             * we'll use something called ORM (object-relational mapper) to handle that with SQL databases.
             * 
             ...
             *      
             * EF Core does come from Microsoft, but it isn't part of CoreFX (the Statndard Libraries)...
             * it's a handful of other assemblies, and .NET Core lets us use NuGet to automatically manage
             * any dependencies like that which we need to downloaad and have connected to our own assemblies
             ...
             * 1. dotnet restore (tells jnuget to download any missing packages)
